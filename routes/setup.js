...
router.get("/sampleData/:id", async (req, res) => {
  try {
    ...
    const correspondents = await paperlessService.getCorrespondentsFromDocument(
      document.id,
    );
  } catch (error) {
...
router.post("/chat/message", async (req, res) => {
  try {
    ...
    if (!documentId || !message) {
      return res
        .status(400)
        .json({ error: "Document ID and message are required" });
    }
...
router.get("/history", async (req, res) => {
  try {
    ...
    const allCorrespondents = [
      ...new Set(historyDocuments.map((doc) => doc.correspondent)),
    ]
      .filter(Boolean)
      .sort();
  ...
  try {
    ...
    let filteredDocs = allDocs
      ...
      .filter((doc) => {
        ...
        const matchesTag =
          !tagFilter || doc.tags.some((tag) => tag.id === parseInt(tagFilter));
...
async function processQueue(customPrompt) {
  ...
  try {
    ...
    const [existingTags, existingCorrespondentList, ownUserId] =
      await Promise.all([
        paperlessService.getTags(),
        paperlessService.listCorrespondentsNames(),
        paperlessService.getOwnUserID(),
...
router.get("/dashboard", async (req, res) => {
  ...
  const processedDocumentCount =
    await documentModel.getProcessedDocumentsCount();
  ...
  try {
    ...
    if (!initSuccess) {
      return res.status(400).json({
        error:
          "Failed to initialize connection to Paperless-ngx. Please check URL and Token.",
      });
    }
    ...
    if (!isPermissionValid.success) {
      return res.status(400).json({
        error:
          "Paperless-ngx API permissions are insufficient. Error: " +
          isPermissionValid.message,
      });
    }
        ...
        for (const field of parsedFields.custom_fields) {
          try {
            ...
            if (createdField) {
              ...
              console.log(
                `[SUCCESS] Created/found custom field: ${field.value}`,
              );
            }
          } catch (fieldError) {
            console.error(
    ...
    const apiToken =
      process.env.API_KEY || require("crypto").randomBytes(64).toString("hex");
    const jwtToken =
...
router.post("/settings", express.json(), async (req, res) => {
  try {
    ...
    if (
      paperlessUrl !== currentConfig.PAPERLESS_API_URL?.replace("/api", "") ||
    ...
    if (aiProvider) {
      ...
      if (aiProvider === "openai" && openaiKey) {
        const isOpenAIValid =
          await setupService.validateOpenAIConfig(openaiKey);
        if (!isOpenAIValid) {
          return res.status(400).json({
            error: "OpenAI API Key is not valid. Please check the key.",
          });
