class ThemeManager {
  ...
  updateShepherdTheme(theme) {
    ...
    activeTooltips.forEach((tooltip) => {
      tooltip.style.background = getComputedStyle(
...
class TagsManager {
  ...
  updateHiddenInput() {
    const tags = Array.from(this.tagsContainer.children).map(
...
class PromptTagsManager {
  constructor() {
    ...
    // Initialize existing tags with click handlers
    document
      .querySelectorAll("#promptTagsContainer .modern-tag button")
      .forEach((button) => {
        button.addEventListener("click", async () => {
          const result = await Swal.fire({
            title: "Remove Tag",
  ...
  initializeFormValidation() {
    ...
    this.form.onsubmit = (e) => {
      if (this.passwordInput.value) {
        // Only validate if password field is present and has a value
...
function createFieldElement(fieldName, data_type, currency = null) {
  ...
  div.className =
    "custom-field-item flex items-center gap-2 p-3 bg-white rounded-lg border border-gray-200 hover:border-blue-200 transition-colors";
...
function addCustomField() {
  ...
  const existingFields = document.querySelectorAll(
    ".custom-field-item p.font-medium",
  );
